...
<header id="header">
 <div class="header_l header_return"> <a class="ico_10" href="seller.php"> 返回 </a> </div>
 <h1> 加油 </h1>
</header>
<div class="blank3"></div>
<section class="wrap">
 <form name="formGas" action="user.php" method="post" >
   <section class="order_box padd1 radius10" style="padding:15px">
     <div class="table_box table_box2">
       <dl>
         <dd class="dd1">支付商家 </dd>
          <td align="left" bgcolor="#ffffff"><?php echo $this->_var['sellerInfo']['name']; ?> (商家账户:<?php echo $this->_var['sellerInfo']['user_name']; ?>)</td>
       </dl>
       <dl>
         <dd class="dd1">消费金额 </dd>
         <input name="amount" type="text"  value="100" class="dd2" />元
       </dl>
       <dl>
         <dd class="dd1">支付方式 </dd>
             <td  align="left">
                 <input style="width:12px" type="radio" name="payment" id="payment_id1" value="money" />
                 <label for="payment_id1">余额支付（可用:<?php echo $this->_var['userInfo']['surplus']; ?>）</label>
             </td><br>
             <td  align="left">
                 <input style="width:12px" type="radio" name="payment" id="payment_id2" value="vip_money" />
                 <label for="payment_id2">VIP余额支付（可用:<?php echo $this->_var['userInfo']['show_vip_money']; ?>）</label>
             </td>
       </dl>
      </div>
   </section>
   <div class="blank3"></div>
   <input type="hidden" name="Verifying" value="<?php echo $this->_var['Verifying']; ?>" />
   <input type="hidden" name="callblock" value="<?php echo $this->_var['callblock']; ?>" />
   <input type="hidden" name="sellerId" value="<?php echo $this->_var['sellerInfo']['user_id']; ?>" />
   <input type="hidden" name="act" value="act_pay_gas" />
   <input name="submit" type="submit" value="确认支付" class="c-btn3"   />
 </form>
</section>
<?php endif; ?>
...
?>
    ...
    <link rel="stylesheet"  href="themes/ocailong_com/css/element-ui.css">
    <style>
        .tip{padding:8px 16px;background-color:#ecf8ff;border-radius:4px;border-left:5px solid #50bfff;margin:0px 0 10px 0}
    ...
    <script src="themes/ocailong_com/js/vue.js"></script>
    <script src="themes/ocailong_com/js/element-ui.js"></script>
    <div id="app" style="margin-top:5px;">
        <el-tabs type="border-card" v-model="activeName" @tab-click="handleClickTab">
          <el-tab-pane name="account_deposit_card">
            <span slot="label"> 充值卡</span>
          </el-tab-pane>
          <el-tab-pane name="account_deposit">
            <span slot="label"> 充值</span>
          </el-tab-pane>
          <el-tab-pane  name="account_detail">
            <span slot="label"> 账户明细</span>
          </el-tab-pane>
          <el-tab-pane  name="account_log">
            <span slot="label"> 申请记录</span>
          </el-tab-pane>
          <?php if ($this->_var['action'] == "account_deposit"): ?>
          <form name="formSurplus" method="post" action="user.php" onSubmit="return submitSurplus()">
             <div class="tip">
                 您的当余额:<?php echo $this->_var['userInfo']['user_money']; ?>
             </div>
             <el-input placeholder="请输入充值金额" name="amount" value="<?php echo htmlspecialchars($this->_var['order']['amount']); ?>">
...
?>
                    <el-radio name="payment_id" v-model="payId" label="<?php echo $this->_var['list']['pay_id']; ?>" border style="margin: 10px 0;">
                        <img src="themes/ocailong_com/images/payimg/<?php echo $this->_var['list']['pay_code']; ?>.png" style="width:99px;height:35px;overflow:hidden;">
                    </el-radio>
                 <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
             </div>
             <div style="margin: 10px 0;">
                 ...
                 <input type="submit" class="c-btn3" style="margin-right:5px;" name="submit" value="<?php echo $this->_var['lang']['submit_request']; ?>" />
                 <input style="margin: 5px 0;" type="reset"  class="c-btn3"  name="reset" value="<?php echo $this->_var['lang']['button_reset']; ?>" /></td>
             </div>
          </form>
          <?php endif; ?>
          ...
          <form name="formSurplus" method="post" action="user.php" onSubmit="return submitSurplus()">
             ...
             <input type="hidden" name="amount" value="<?php echo $this->_var['faceValue']; ?>" />
             <el-input placeholder="<?php echo $this->_var['faceValue']; ?>" name="amounts" value="<?php echo $this->_var['faceValue']; ?>" :disabled="true">
                <template slot="prepend">面值</template>
                <template slot="append">到账<?php echo $this->_var['accountAmount']; ?>元</template>
             </el-input>
             <div style="margin: 10px 0;"></div>
                 <el-tag>数量：</el-tag>
                 <el-input-number v-model="cardNumber" name="number" :min="1" :max="1000" label="充值卡数量" @change="handleChangeCardNumber"></el-input-number>
             <div>
                 <?php $_from = $this->_var['payment']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'list');if (count($_from)):
    foreach ($_from AS $this->_var['list']):
?>
...
?>
     ...
     <script>
         var Main = {
            data() {
              return {
                activeName: '<?php echo $this->_var['action']; ?>',
                cardNumber: 1,
                payId: '<?php echo $this->_var['payment']['0']['pay_id']; ?>'
              };
            },
            methods: {
                handleClickTab(tab, event) {
                    //跳转到指定页面
                    location.href='user.php?act='+tab.name
                },
...
<section class="wrap">
   ...
   <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table">
    ...
    <?php if ($this->_var['action'] == "nav_list"): ?>

        <div id="nav_list"><section class="order_box padd1 radius10"> 定位中。。。    </section></div>
    ...
    <script>
      ...
      wx.ready(function () {
        ...
        success: function (res) {
            ...
            $.ajax({
                type: "POST",
                url: "user.php?act=ajxa_nav_list",
                data: "latitude="+latitude+"&longitude="+longitude,
                dataType: "html",
                async: false,
                success: function(result){
                    // console.log(result);
                     $("#nav_list").html( result );
                    // if (result.code!=2){
                    //     alert(result.msg);
                    //     status = false;
                    // }
