    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type)) {
        ...
        if (is_pups($recid)) {
            if (isset($consignee['city']) && intval($consignee['city'])>0) {
                ...
                if ($pickinfo) {
                    $result['picktxt'] = $pickinfo[0]['shop_name']."<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pickinfo[0]['id']."'>";
                    // $result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pickinfo[0]['id']."'><span style='color:#F30'>自提点：</span><span>".$pickinfo[0]['shop_name']."</span><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' style='color:#F30;margin-left:40px'>修改</a>";
                }
                foreach ($pickinfo as $pkey=>$pval) {
                    if ($consignee['district'] == $pval['district_id']) {
                        $result['picktxt'] = $pval['shop_name']."<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pval['id']."'>";
                        // $result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pval['id']."'><span style='color:#F30'>自提点：</span><span>".$pval['shop_name']."</span><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' style='color:#F30;margin-left:40px'>修改</a>";
                    }
                }
            }
        }
