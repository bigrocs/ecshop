...
( '$fromUsername', '$menu_message', $time);");
            ...
            if ($keyword == 'ewm') {
                ...
                if (!file_exists($image1) || !is_dir($image1)) {
                    mkdir($image1, 0777);
                }
                if (!file_exists($image2) || !is_dir($image2)) {
                    mkdir($image2, 0777);
                }
                if (!file_exists($image3) || !is_dir($image3)) {
                    mkdir($image3, 0777);
                }
                if (!file_exists($image4) || !is_dir($image4)) {
                    mkdir($image4, 0777);
                }
                if (!file_exists($image5) || !is_dir($image5)) {
                    mkdir($image5, 0777);
                }
...
nation=大雁塔&mode=driving&region=西安&output=html';
    ...
    //新增
    public function htmltowei($contents)
    {
    ...
    /*模板之家*/
    public function downloadimageformweixin($url)
    {
        $ch = curl_init();
