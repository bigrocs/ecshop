 ...
 * ============================================================================
 * $Author: liuwave $
 * $Id: weixin.php 17217 2011-01-19 06:29:08Z douqinghua $
 */
...
if (isset($set_modules) && $set_modules == true) {
    ...
    /* 作者 */
    $modules[$i]['author']  = 'bigrocs QQ:532388887';

    /* 网址 */
    $modules[$i]['website'] = 'http://www.bigrocs.vip';
...
class weixin
{
    ...
    public function get_code($order, $payment)
    {
        ...
        $payment_path=$root.'includes/modules/payment/weixin/';


        $javascript='<style>#paymentDiv{width:760px}#wxPhone{float:left;width:379px;height:421px;padding-left:50px;background:url('.$payment_path.'phone-bg.png) 50px 0 no-repeat}#qrcode{display:block;float:left;margin-top:30px}#qrcode img{height:260px;width:260px;padding:5px;border:1px solid #ddd}#qrcode p{padding:15px 0;background:#E31939;color:#fff;margin:10px 0}</style> ';
        ...
        if (JS_QR) {
            ...
            require_once('weixin/cls_qrcode.php');
        ...
        $javascript.='<script>function getInterval(){Ajax.call("respondwx.php?check=true&out_trade_no='.$this->parameters["out_trade_no"].'&time="+new Date().getTime(),"", function(result){
            if(result.error>0 && result.error<100){ setTimeout(function(){getInterval();},'.(QUERY_INTERVAL*1000).');new Date().getTime();
            }else{location="/respondwx.php?check=true&redirect=true&out_trade_no='.$this->parameters["out_trade_no"].'";}}, "POST", "JSON");
    ...
    public function getCodeUrl()
    {
        ...
        $this->parameters["mch_id"] = $this->payment['partnerId']; // 商户号
    ...
    public function getSign($Obj)
    {
        ...
        // 签名步骤二：在string后加入KEY
        $String = $String . "&key=" . $this->payment['partnerKey'];
