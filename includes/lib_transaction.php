    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        if ($row['order_status'] == OS_UNCONFIRMED) {
            ...
            if ($row['shipping_status'] == SS_SHIPPED) {
                $querenshouhuo = $row['is_pickup']?'领取货物':$GLOBALS['_LANG']['received'];
                @$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$querenshouhuo."</a>";
