...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
    ...
    //厂家信息
    create_html_editor('seller_desc', $goods['seller_desc'], 'Seller');

    $regions = analysisRegions($goods['goods_id']);//售卖区域数据编译
    /* 模板赋值 */
    $smarty->assign('code', $code);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
    ...
    $regions = setRegions($goods_id);//设置商品地区
...
function setRegions($goodsId)
{
    ...
    foreach ($regions as $key => $val) {
        $sql = "INSERT INTO ".$GLOBALS['ecs']->table('goods_region')." (goods_id, region_id) VALUES ('$goodsId', '$val')";
        $GLOBALS['db']->query($sql);
    }
    return $regions;
}
/**
 * [getRegions 获取收请求中的售卖区域]
...
/**
 * [analysisRegions 编译售卖区域数据]
 * @param  [type]  $goods [description]
 * @return boolean        [description]
 */
function analysisRegions($goodsId)
{
    $returne = array();
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('goods_region'). " WHERE goods_id='$goodsId'";
    $res = $GLOBALS['db']->query($sql);
    while ($arr = $GLOBALS['db']->fetchRow($res)) {
        $sql = "SELECT region_name FROM " .$GLOBALS['ecs']->table('region'). " WHERE region_id='$arr[region_id]'";
        $returne[$arr['region_id']] = $GLOBALS['db']->getOne($sql);
    }
    return $returne;
}
