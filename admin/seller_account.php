 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: seller_account.php 17217 2011-01-19 06:29:08Z liubo $
*/
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('ur_here', '商家'.$_LANG['09_user_account']);
    $smarty->assign('id', $user_id);
    $smarty->assign('payment_list', $payment);
    $smarty->assign('action_link', array('text' => $_LANG['surplus_add'], 'href'=>'seller_account.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('seller_account_list.htm');
}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
    ...
    if ($_REQUEST['act'] == 'edit') {
        /* 取得余额信息 */
        $user_account = $db->getRow("SELECT * FROM " .$ecs->table('user_seller_account') . " WHERE id = '$id'");
    ...
    if ($_REQUEST['act'] == 'add') {
        $href = 'seller_account.php?act=list';
    } else {
        $href = 'seller_account.php?act=list&' . list_link_postfix();
    }
    $smarty->assign('action_link', array('href' => $href, 'text' => $_LANG['09_user_account']));

    assign_query_info();
    $smarty->display('seller_account_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
    ...
    if ($_REQUEST['act'] == 'insert') {
        /* 入库的操作 */
        if ($process_type == 1) {
            $surplus = sellerCash($user_id, $amount);//卖家提现
        } else {
            $sql = "INSERT INTO " .$ecs->table('user_seller_account').
        ...
        /* 更新数据表 */
        $sql = "UPDATE " .$ecs->table('user_seller_account'). " SET ".
    ...
    $link[1]['href'] = 'seller_account.php?act=add';
...
elseif ($_REQUEST['act'] == 'check') {
    ...
    /* 如果参数不合法，返回 */
    if ($id == 0) {
        ecs_header("Location: seller_account.php?act=list\n");
        exit;
    }

    /* 查询当前的预付款信息 */
    $account = array();
    $account = $db->getRow("SELECT * FROM " .$ecs->table('user_seller_account'). " WHERE id = '$id'");
    ...
    $smarty->assign('user_name', $user_name);
    $smarty->assign('sellerInfo', getSellerInfo($account['user_id']));//提现卖家信息
    $smarty->assign('id', $id);
    ...
    /* 页面显示 */
    assign_query_info();
    $smarty->display('seller_account_check.htm');
}
...
elseif ($_REQUEST['act'] == 'action') {
    ...
    if ($account['is_paid'] == 0) {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1') {
            ...
            logSellerAccountChange($account['user_id'], $account['user_id'], 0, '提现完成', ACT_DRAWING, $amount);//确认提现资金
        } elseif ($is_paid == '1' && $account['process_type'] == '0') {
            //如果是预付款，并且已完成, 更新此条记录，增加相应的余额
            update_user_account($id, $amount, $admin_note, $is_paid);
            //更新会员余额数量
            logSellerAccountChange($account['user_id'], $account['user_id'], $amount, $_LANG['surplus_type_0'], ACT_SAVING);
        } elseif ($is_paid == '0') {
            /* 否则更新信息 */
            $sql = "UPDATE " .$ecs->table('user_seller_account'). " SET ".
        ...
        /* 提示信息 */
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'seller_account.php?act=list&' . list_link_postfix();
...
elseif ($_REQUEST['act'] == 'query') {
    ...
    make_json_result($smarty->fetch('seller_account_list.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax删除一条信息
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'remove') {
    ...
    refundCash($id);//如果订单是提现退款到卖家余额里面
    $sql = "SELECT u.user_name FROM " . $ecs->table('users') . " AS u, " .
           $ecs->table('user_seller_account') . " AS ua " .
           " WHERE u.user_id = ua.user_id AND ua.id = '$id' ";
    $user_name = $db->getOne($sql);
    $sql = "DELETE FROM " . $ecs->table('user_seller_account') . " WHERE id = '$id'";
    if ($db->query($sql, 'SILENT')) {
        admin_log(addslashes($user_name), 'remove', 'user_surplus');
        $url = 'seller_account.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
function account_list()
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('user_seller_account'). " AS ua, ".
                   $GLOBALS['ecs']->table('users') . " AS u " . $where;
        ...
        $sql  = 'SELECT ua.*, u.user_name FROM ' .
            $GLOBALS['ecs']->table('user_seller_account'). ' AS ua LEFT JOIN ' .
